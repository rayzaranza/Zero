...
void ExampleLayer::OnAttach() {
  ...
  const Zero::VertexBufferLayout layout{
    { Zero::VertexAttributeType::Vector3, "a_Position" },
    { Zero::VertexAttributeType::Vector2, "a_UV" },
  };

  const Zero::Ref<Zero::VertexBuffer> quadVertexBuffer{ Zero::VertexBuffer::Create(
    {
  ...
  const Zero::Ref<Zero::IndexBuffer> quadIndexBuffer{ Zero::IndexBuffer::Create({ 0, 1, 2, 2, 3, 0 }) };
  m_QuadVertexArray->SetIndexBuffer(quadIndexBuffer);

  m_ShaderLibrary->Load("D:/Zero/Sandbox/assets/shaders/Texture.glsl");
  const Zero::Ref<Zero::Shader> textureShader{ m_ShaderLibrary->Get("Texture") };
  textureShader->Bind();
