...
static bool ImGui_ImplGlfw_Init(GLFWwindow* window, bool install_callbacks, GlfwClientApi client_api)
{
    ...
    snprintf(bd->BackendPlatformName, sizeof(bd->BackendPlatformName), "imgui_impl_glfw (%d)%s", GLFW_VERSION_COMBINED,
             bd->IsWayland ? " (Wayland)" : "");
...
static void ImGui_ImplGlfw_UpdateMouseCursor()
{
    ...
    {
        bd->LastMouseCursor =
            nullptr; // Invalidate so that if user changes underlying cursor we will update it next time we can.
        return;
    }
    ...
    // (those braces are here to reduce diff with multi-viewports support in 'docking' branch)
    {
        ...
        else
        {
            ...
            // here.
    #if GLFW_HAS_CREATECURSOR
            GLFWcursor* cursor = bd->MouseCursors[imgui_cursor] ? bd->MouseCursors[imgui_cursor]
...
static void ImGui_ImplGlfw_UpdateGamepads()
{
    ...
    // ImGui_ImplWin32_GetDpiScaleForMonitor(). This is merely a band-aid to make using GLFW 3.2 a little bit nicer, but
    // prefer to use GLFW 3.3+ or the full correct functions from the Win32 backend.
